...
#define CHECK_MUSA(tensor) {\
    TORCH_CHECK((tensor).device().type() == c10::DeviceType::PrivateUse1, #tensor " is not in MUSA memory"); \
    TORCH_CHECK((tensor).is_contiguous(), #tensor " is not contiguous"); }
void CHECK_KERNEL() {auto error = musaGetLastError(); TORCH_CHECK( error == musaSuccess, musaGetErrorString(error));}


template < typename scalar_t  >
__global__ void rope_2d_musa_kernel(
...
void rope_2d_musa( torch::Tensor tokens, const torch::Tensor pos, const float base, const float fwd )
{
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::kHalf, at::kBFloat16, tokens.scalar_type(), "rope_2d_musa", ([&] {
        rope_2d_musa_kernel<scalar_t> <<<N_BLOCKS, THREADS_PER_BLOCK, SHARED_MEM>>> (

...
// forward declaration
void rope_2d_musa( torch::Tensor tokens, const torch::Tensor pos, const float base, const float fwd );
        ...
        const float fwd )
{
    ...
    const bool tokens_is_musa = tokens.device().type() == c10::DeviceType::PrivateUse1;
    const bool positions_is_musa = positions.device().type() == c10::DeviceType::PrivateUse1;
    TORCH_CHECK(tokens_is_musa == positions_is_musa, "tokens and positions are not on the same device" );
    TORCH_CHECK(tokens.device().type() == positions.device().type(), "tokens and positions are not on the same device" );
    TORCH_CHECK(tokens.device().index() == positions.device().index(), "tokens and positions are not on the same device" );

    if (tokens_is_musa)
        rope_2d_musa( tokens, positions, base, fwd );
    else

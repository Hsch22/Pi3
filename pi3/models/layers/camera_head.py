...
from pi3.utils.device import disabled_autocast
from pi3.utils.rotation import (
    det3,
    polar_project_so3,
    rotation_error_mask,
    svd_project_so3,
...
class CameraHead(nn.Module):
    ...
    def forward(self, feat, patch_h, patch_w):
        ...
        with disabled_autocast(feat.device):
    ...
    def svd_orthogonalize(self, m, polar_iters=12, orth_threshold=1e-3, det_threshold=1e-3):
        """Convert 9D representation to SO(3).
        ...
        device = m.device
        dtype = m.dtype
        rotation_input = F.normalize(m, p=2, dim=-1)

        if device.type != "musa":
            return svd_project_so3(rotation_input).to(device=device, dtype=dtype)

        r = polar_project_so3(rotation_input, num_iters=polar_iters)
        bad, _, _ = rotation_error_mask(r, orth_threshold=orth_threshold, det_threshold=det_threshold)
        bad = bad | (det3(rotation_input.float()) < 0)
        if bad.any():
            r = r.clone()
            r[bad] = svd_project_so3_cpu(rotation_input[bad], device=device, dtype=r.dtype)

#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="${PI3_PROJECT_ROOT:-$(cd "${SCRIPT_DIR}/.." && pwd)}"
VENV_DIR="${PI3_VENV_DIR:-${PROJECT_ROOT}/.venv}"
IMAGE_TAG="${PI3_IMAGE:-registry.mthreads.com/mcctest/ai/mtwan:4.3.3-pt2.7-v0.2-mudnn3.1.7-ph1}"
SHARED_IMAGE_LOADER="${PI3_SHARED_IMAGE_LOADER:-/datapool/shared_images/mtwan_4.3.3-pt2.7-v0.2-mudnn3.1.7-ph1/load_mtwan.sh}"
PYPI_MIRROR="${PI3_PYPI_MIRROR:-https://pypi.tuna.tsinghua.edu.cn/simple}"
PYPI_FALLBACK="${PI3_PYPI_FALLBACK:-https://pypi.org/simple}"
PROXY_ENV="${PI3_PROXY_ENV:-/datapool/.config/mihomo/proxy-env.sh}"
REQ_FILE="${PI3_REQUIREMENTS:-${PROJECT_ROOT}/requirements-musa.txt}"
if [ "${PI3_INSTALL_DEMO:-0}" = "1" ]; then
  REQ_FILE="${PROJECT_ROOT}/requirements-demo-musa.txt"
fi
LOCK_FILE="${PI3_VENV_LOCK_FILE:-${VENV_DIR}.lock}"
STAMP_FILE="${VENV_DIR}/.pi3-musa-env.sha256"

in_mtwan_image() {
  python3 - <<'PY' >/dev/null 2>&1
import importlib.util
import sys

if sys.version_info[:2] != (3, 10):
    raise SystemExit(1)
if importlib.util.find_spec("torch_musa") is None:
    raise SystemExit(1)
PY
}

run_in_container() {
  if ! docker image inspect "${IMAGE_TAG}" >/dev/null 2>&1; then
    bash "${SHARED_IMAGE_LOADER}"
  fi

  docker run --rm -i \
    --privileged \
    --network=host \
    --ipc=host \
    --runtime mthreads \
    -v /datapool:/datapool \
    -w "${PROJECT_ROOT}" \
    -e PI3_BOOTSTRAP_IN_CONTAINER=1 \
    -e PI3_PROJECT_ROOT="${PROJECT_ROOT}" \
    -e PI3_VENV_DIR="${VENV_DIR}" \
    -e PI3_PYPI_MIRROR="${PYPI_MIRROR}" \
    -e PI3_PYPI_FALLBACK="${PYPI_FALLBACK}" \
    -e PI3_PROXY_ENV="${PROXY_ENV}" \
    -e PI3_REQUIREMENTS="${REQ_FILE}" \
    -e PI3_INSTALL_DEMO="${PI3_INSTALL_DEMO:-0}" \
    -e UV_LINK_MODE="${UV_LINK_MODE:-copy}" \
    -e PI3_HOST_UID="$(id -u)" \
    -e PI3_HOST_GID="$(id -g)" \
    "${IMAGE_TAG}" \
    bash -lc "bash scripts/bootstrap_musa_uv.sh"
}

find_uv() {
  if [ -x "${VENV_DIR}/bin/python" ] && "${VENV_DIR}/bin/python" -m uv --version >/dev/null 2>&1; then
    UV_CMD=("${VENV_DIR}/bin/python" -m uv)
  elif command -v uv >/dev/null 2>&1; then
    UV_CMD=(uv)
  elif [ -x "${HOME:-/root}/.local/bin/uv" ]; then
    UV_CMD=("${HOME:-/root}/.local/bin/uv")
  elif python3 -m uv --version >/dev/null 2>&1; then
    UV_CMD=(python3 -m uv)
  else
    return 1
  fi
}

with_proxy_fallback() {
  if "$@"; then
    return 0
  fi
  if [ -f "${PROXY_ENV}" ]; then
    # shellcheck disable=SC1090
    source "${PROXY_ENV}"
  fi
  "$@"
}

install_uv_user() {
  with_proxy_fallback env PYTHONNOUSERSITE= python3 -m pip install --user -i "${PYPI_MIRROR}" uv
}

acquire_lock() {
  mkdir -p "$(dirname "${LOCK_FILE}")"
  exec 9>"${LOCK_FILE}"
  flock 9
}

venv_uses_system_site_packages() {
  [ -f "${VENV_DIR}/pyvenv.cfg" ] \
    && grep -Eq '^include-system-site-packages[[:space:]]*=[[:space:]]*true$' "${VENV_DIR}/pyvenv.cfg"
}

validate_runtime() {
  "${VENV_DIR}/bin/python" - <<'PY'
import importlib
import sys

expected = {
    "torch": "2.7.1",
    "torch_musa": "2.7.1+9f1bb31",
    "torchvision": "0.22.1+32091f2",
}

errors = []
for module_name, version in expected.items():
    try:
        module = importlib.import_module(module_name)
    except Exception as exc:
        errors.append(f"{module_name}: import failed: {exc!r}")
        continue
    actual = getattr(module, "__version__", None)
    if actual != version:
        errors.append(f"{module_name}: expected {version}, got {actual}")

try:
    import torch
    import torch_musa  # noqa: F401
    if not torch.musa.is_available():
        errors.append("torch.musa.is_available() is false")
except Exception as exc:
    errors.append(f"torch.musa check failed: {exc!r}")

if errors:
    print("Pi3 MUSA runtime validation failed:", file=sys.stderr)
    for error in errors:
        print(f"  - {error}", file=sys.stderr)
    raise SystemExit(1)
PY
}

validate_no_managed_stack_in_venv() {
  "${VENV_DIR}/bin/python" - <<'PY'
import importlib.metadata as metadata
import sys
import sysconfig

site = sysconfig.get_paths()["purelib"]
blocked_names = {
    "torch",
    "torchvision",
    "torch-musa",
    "torchaudio",
    "triton",
    "opencv-python",
}
blocked = []
for dist in metadata.distributions(path=[site]):
    name = dist.metadata.get("Name", "").lower().replace("_", "-")
    if name in blocked_names or name.startswith("nvidia-") or name.startswith("cuda-"):
        blocked.append(name)

if blocked:
    print("Blocked packages were installed into .venv:", file=sys.stderr)
    for name in sorted(blocked):
        print(f"  - {name}", file=sys.stderr)
    raise SystemExit(1)
PY
}

validate_app_imports() {
  "${VENV_DIR}/bin/python" - <<'PY'
import importlib
import sys

modules = [
    "cv2",
    "huggingface_hub",
    "modelscope",
    "numpy",
    "PIL",
    "pi3",
    "plyfile",
    "safetensors",
]

errors = []
for module_name in modules:
    try:
        importlib.import_module(module_name)
    except Exception as exc:
        errors.append(f"{module_name}: {exc!r}")

if errors:
    print("Pi3 application import validation failed:", file=sys.stderr)
    for error in errors:
        print(f"  - {error}", file=sys.stderr)
    raise SystemExit(1)
PY
}

env_stamp() {
  sha256sum \
    "${PROJECT_ROOT}/pyproject.toml" \
    "${PROJECT_ROOT}/requirements-musa.txt" \
    "${PROJECT_ROOT}/requirements-demo-musa.txt" \
    "${PROJECT_ROOT}/constraints-musa.txt" \
    "${PROJECT_ROOT}/excludes-musa.txt" \
    "${PROJECT_ROOT}/scripts/bootstrap_musa_uv.sh" \
    | sha256sum \
    | awk '{print $1}'
}

uv_pip_install() {
  if "${UV_CMD[@]}" pip install \
    --python "${VENV_DIR}/bin/python" \
    --index-url "${PYPI_MIRROR}" \
    --index-strategy unsafe-best-match \
    "$@"; then
    return 0
  fi
  if [ -f "${PROXY_ENV}" ]; then
    # shellcheck disable=SC1090
    source "${PROXY_ENV}"
  fi
  "${UV_CMD[@]}" pip install \
    --python "${VENV_DIR}/bin/python" \
    --index-url "${PYPI_FALLBACK}" \
    --index-strategy unsafe-best-match \
    "$@"
}

if [ "${PI3_BOOTSTRAP_IN_CONTAINER:-0}" != "1" ] && ! in_mtwan_image; then
  run_in_container
  exit $?
fi

cd "${PROJECT_ROOT}"

export PATH="${HOME:-/root}/.local/bin:${PATH}"
export UV_LINK_MODE="${UV_LINK_MODE:-copy}"

acquire_lock

if ! find_uv; then
  install_uv_user
  find_uv
fi

if [ ! -x "${VENV_DIR}/bin/python" ] || ! venv_uses_system_site_packages; then
  rm -rf "${VENV_DIR}"
  "${UV_CMD[@]}" venv --python /usr/bin/python3 --system-site-packages "${VENV_DIR}"
fi

uv_pip_install uv
UV_CMD=("${VENV_DIR}/bin/python" -m uv)

validate_runtime

CURRENT_STAMP="$(env_stamp)"
INSTALLED_STAMP=""
if [ -f "${STAMP_FILE}" ]; then
  INSTALLED_STAMP="$(cat "${STAMP_FILE}")"
fi

if [ "${CURRENT_STAMP}" != "${INSTALLED_STAMP}" ] || ! validate_no_managed_stack_in_venv || ! validate_app_imports; then
  uv_pip_install \
    -e "${PROJECT_ROOT}" \
    -r "${REQ_FILE}" \
    --constraints "${PROJECT_ROOT}/constraints-musa.txt" \
    --excludes "${PROJECT_ROOT}/excludes-musa.txt"
  printf '%s\n' "${CURRENT_STAMP}" > "${STAMP_FILE}"
fi

validate_runtime
validate_no_managed_stack_in_venv
validate_app_imports

if [ -n "${PI3_HOST_UID:-}" ] && [ -n "${PI3_HOST_GID:-}" ]; then
  chown -R "${PI3_HOST_UID}:${PI3_HOST_GID}" "${VENV_DIR}" "${LOCK_FILE}" 2>/dev/null || true
  if [ -d "${PROJECT_ROOT}/pi3.egg-info" ]; then
    chown -R "${PI3_HOST_UID}:${PI3_HOST_GID}" "${PROJECT_ROOT}/pi3.egg-info" 2>/dev/null || true
  fi
fi

echo "Pi3 MUSA uv environment ready: ${VENV_DIR}"
